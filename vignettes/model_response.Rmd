...
`parsnip` provides a tidyverse style uniform interface for training and calling a lot of machine learning models. `easyalluvial` provides a function that wraps the above described workflow into one single call using the `vip` package to get the feature importance. 

```{r fig.height = 8, fig.width=10, cache = T}

m_parsnip = parsnip::rand_forest(mode = "regression") %>%
  parsnip::set_engine("randomForest") %>%
  parsnip::fit(lstat ~., df) 

alluvial_model_response_parsnip(m_parsnip, df, degree = 4, bins = 5
                              , stratum_label_size = 2.8)
...
`caret` similar to `parsnip` provides a uniform modeling syntax and also returns feature importance.
...
thanks to the uniform modeling interface of `caret` and `parsnip` `alluvial_model_response_caret()`  and `alluvial_model_response_parsnip()` can call `get_pdp_predictions()` directly.

```{r pdp_caret, fig.height = 8, fig.width=10, cache = T, eval = FALSE}
